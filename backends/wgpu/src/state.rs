...
use wgpu::{
    util::DeviceExt, Device, Instance, Queue, RenderPipeline, Surface, SwapChain,
    SwapChainDescriptor,
};
...
impl GpuState {
    pub(super) async fn new(window: &Window) -> Result<Self, Error> {
        ...
        let swap_chain_descriptor = SwapChainDescriptor {
            ...
            width: u32::try_from(size.width).unwrap(),
            height: u32::try_from(size.height).unwrap(),
        ...
        let vs_module = device.create_shader_module(&wgpu::ShaderModuleDescriptor {
            ...
            source: wgpu::util::make_spirv(include_bytes!("shader.vert.spv")),
    ...
    pub fn resize(&mut self, new_size: PhysicalSize<u32>) {
        self.size = new_size;
        self.swap_chain_descriptor.width = u32::try_from(new_size.width).unwrap();
        self.swap_chain_descriptor.height = u32::try_from(new_size.height).unwrap();
